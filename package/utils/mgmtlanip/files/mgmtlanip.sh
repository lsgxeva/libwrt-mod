...
_fix_mgmtlanip()
{
	...
	do
		if [ "$ifname" = "${lan_ifname}:${lan_ifidx}" ]; then
			is_lan=1
			route_count=$( ip addr show ${lan_ifname} | grep inet | grep -v inet6 | wc -l )
			[ $route_count -eq 1 ] && is_first=1
			[ $route_count -ge 2 ] && is_first=$( ip addr show ${ifname} | grep inet | grep -m 1 -v inet6 | grep ${lan_ipad}/${lan_mask} | wc -l )
			[ $is_first -eq 1 ] && ip addr del ${lan_ipad}/${lan_mask} dev ${ifname} && is_lan=0 && 
				logg "* _fix_mgmtlanip - del first item - ip addr del ${lan_ipad}/${lan_mask} dev ${ifname} "
		else
			[ ! -z "$ifname" ] && ip addr del ${lan_ipad}/${lan_mask} dev ${ifname} && is_lan=0 && 
				logg "* _fix_mgmtlanip - del other item - ip addr del ${lan_ipad}/${lan_mask} dev ${ifname} "
		fi
	done

	route_count=$( ip addr show ${lan_ifname} | grep inet | grep -v inet6 | wc -l )
	[ $route_count -ge 1 ] && [ $is_lan -eq 0 ] && 
		[ ! -z "$lan_ifname" ] && ip addr add ${lan_ipad}/${lan_mask} brd ${lan_brd} dev ${lan_ifname} label ${lan_ifname}:${lan_ifidx} && is_lan=1 && 
		logg "* _fix_mgmtlanip - ip addr add ${lan_ipad}/${lan_mask} brd ${lan_brd} dev ${lan_ifname} label ${lan_ifname}:${lan_ifidx} "
